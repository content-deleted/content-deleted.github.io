...
Scene_Birdle.prototype.initialize = function () {
    ...
    $gameSystem._birdle_phase = 1; // start phase
    $gameSystem.wordChecker = {checking: false, done: false, res: false};
...
Scene_Birdle.prototype.createScoreText = function () {
    ...
    this._scoretext_sprite.x = Graphics.boxWidth / 2 + 20;
    this._scoretext_sprite.y = 200;
...
Scene_Birdle.prototype.update_end_phase = function () {
    switch (this._phase_state) {
        case 0:
            ...
            this._scoretext_sprite.bitmap.drawText("Answer:\n"+this._tile_manager._answer, 0, 0, 200, 48, "center");
            this._scoretext_sprite.opacity = 0;
            this._results_sprite.opacity = 0;
            this._phase_state = 1;
            break;
        case 1:
            this._results_sprite.opacity += 3;
            this._scoretext_sprite.opacity += 3;
...
Sprite_TileManager.prototype.update = function () {
    ...
    if(Input.isTriggered("#enter")) {
        if(this._letters.length == this.Letters) {
            let ans = this._letters.join("").toUpperCase();
            if(Sprite_TileManager.words.contains(ans)) {
                // do the wordle thing
                this._checking = true;
                this._flip_timer = Sprite_TileManager.flipTimerMax;
                this.check_pointer = 0;
            } else {
                this.checkWordExists(ans)
            }
        } else {
            // print message 
        }
    ...
    if($gameSystem.wordChecker.checking && $gameSystem.wordChecker.done) {
        if($gameSystem.wordChecker.res) {
            this._checking = true;
            this._flip_timer = Sprite_TileManager.flipTimerMax;
            this.check_pointer = 0;
        } else {
...
Sprite_TileManager.prototype.checkWordExists = function (word) {
    $gameSystem.wordChecker.checking = true;
    checkWordRequestWrapper(word).then(function(data) {
        $gameSystem.wordChecker = {checking: true, done: true, res: true};
    }).catch(function(err) {
        $gameSystem.wordChecker = {checking: true, done: true, res: false};
    });
};

function checkWordRequestWrapper(word) {
    return new Promise(function(resolve, reject) {
        const url = "https://api.wordnik.com/v4/word.json/" + word.toLowerCase() + "/definitions?limit=200&includeRelated=false&useCanonical=false&includeTags=false&api_key=a2a73e7b926c924fad7001ca3111acd55af2ffabf50eb4ae5";
...
Sprite_TileManager.getWordOfTheDay = function() {
    var now = new Date();
